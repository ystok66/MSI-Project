...
from .objectives import Coord, Inventory, Objective, ObjectiveEvent, ObjectiveState
...
class MazeState:
    ...
    trap_types: dict[Coord, int]
    risk_latent_features: dict[Coord, np.ndarray]
...
class POMDPConfigView:
    ...
    def from_any(cls, config: Any | None) -> "POMDPConfigView":
        if config is None:
            return cls()
        return cls(
            risk_dim=int(getattr(config, "risk_dim", cls.risk_dim)),
            n_safe_types=int(getattr(config, "n_safe_types", cls.n_safe_types)),
            n_trap_types=int(getattr(config, "n_trap_types", cls.n_trap_types)),
            cluster_std=float(getattr(config, "cluster_std", cls.cluster_std)),
            obs_noise=float(getattr(config, "obs_noise", cls.obs_noise)),
            view_radius=int(getattr(config, "view_radius", cls.view_radius)),
            hp=int(getattr(config, "hp", cls.hp)),
        )
...
class RiskyMazePOMDPEnv:
    ...
    Hidden true labels remain inside ``state.layout`` and are never copied into
    ...
    def __init__(
        ...
        *,
        ...
        self._class_means = self._make_class_means()
        self.risk_latent_features = self._make_latent_features()
    ...
    def reset(self, task: MazeTask | None = None, seed: int | None = None) -> Observation:
        ...
        self.state = MazeState(
            ...
            layout=self.layout,
            trap_types=dict(self.trap_types),
            risk_latent_features=self.risk_latent_features,
        )
    ...
    def step(self, action: Any) -> tuple[Observation, StepOutcome, bool, bool, dict[str, Any]]:
        ...
        if moved and self.layout.is_trap(new_pos):
            ...
            danger_feature = self.risk_latent_features.get(new_pos)
    ...
    def _observe(self) -> Observation:
        ...
        return Observation(
            ...
            objective_index=self.objective_state.index,
            current_objective=self.objective_state.current(),
            objective_sequence=list(self.task.objectives),
            has_gem_or_collected=set(self.state.collected_gems),
    ...
    def _make_latent_features(self) -> dict[Coord, np.ndarray]:
        feats: dict[Coord, np.ndarray] = {}
        for coord in self.layout.walkable_cells():
            class_id = min(self.layout.latent_class_id(coord), len(self._class_means) - 1)
            feats[coord] = self._class_means[class_id] + self._prototype_rng.normal(
    ...
    def _ignore_trap_damage(self) -> bool:
        return self.baseline_mode in {"immortal_warnlike", "immortal_no_timeout"}
