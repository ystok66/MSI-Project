...
from .objectives import Coord, Objective, parse_objective_sequence, parse_runtime_coord
...
class FixedRuntimeLayout:
    ...
    -----------------------------------------
    The inverse-planning tutor's A* hot path calls ``is_walkable``,
    ``in_bounds``, and ``height``/``width`` tens of millions of times per
    episode.  The original implementation traversed ``len(self.rows)`` on
    every property access — profiling showed 69M ``len()`` calls consuming
    ~20 s of a 53 s job.

    Three pre-computed caches eliminate this overhead:

    * ``_h`` / ``_w``: grid dimensions cached once at construction, making
      ``.height`` and ``.width`` zero-cost attribute reads.
    * ``_walkable_coords``: a ``frozenset[Coord]`` of all non-wall cells,
      enabling ``is_walkable`` and ``shadow_traversable`` to use a single
      ``coord in frozenset`` check (O(1)) instead of the former
      ``in_bounds → char_at → WALL_CHARS`` call chain.

    These caches are safe because the layout is immutable (``frozen=True``).
    The original A* interface (``shortest_path``, ``is_wall``, ``char_at``)
    is fully preserved — the caches are an additive fast-path that downstream
    code can detect via ``getattr(layout, '_walkable_coords', None)``.
    """

    rows: tuple[str, ...]
    name: str = "fixed"
    cell_features: dict[Coord, Any] | None = field(default=None, compare=False, hash=False, repr=False)

    # --- Pre-computed performance caches (set once in __post_init__) ---
    # Dimensions: avoids len(self.rows) on every .height/.width property call.
    _h: int = 0
    _w: int = 0
    # Walkable coordinate set: enables O(1) is_walkable / shadow_traversable.
    # Stored as frozenset for immutability and fast ``in`` checks.
    _walkable_coords: frozenset[Coord] | None = field(default=None, compare=False, hash=False, repr=False)
    ...
    def __post_init__(self) -> None:
        ...
        # Cache dimensions and walkable set.  Since this is a frozen dataclass,
        # we must use object.__setattr__ to write during __post_init__.
        object.__setattr__(self, '_h', len(self.rows))
        object.__setattr__(self, '_w', width)
        walkable = frozenset(
            (r, c) for r in range(len(self.rows)) for c in range(width)
            if self.rows[r][c] not in WALL_CHARS
        )
        object.__setattr__(self, '_walkable_coords', walkable)
    ...
    def is_walkable(self, coord: Coord) -> bool:
        return coord in self._walkable_coords
...
def parse_task(task: Any, *, task_id: str | None = None, split: str = "teach") -> MazeTask:
    ...
    start = parse_runtime_coord(_first_present(d, "start", "start_coord", "initial_pos", "pos"))
...
def _normalise_spec(raw: Any, *, name: str) -> FixedMazeSpec:
    ...
    nested_tasks = _first_present(d, "tasks", "task_suite", "episodes")
    nested_task_dict = _as_dict(nested_tasks) if isinstance(nested_tasks, dict) else {}

    teach_tasks = _extract_task_dict(d, preferred_keys=("teach_tasks", "train_tasks", "teach", "train"))
    if not teach_tasks and nested_task_dict:
        teach_tasks = _extract_task_dict(nested_task_dict, preferred_keys=("teach", "train", "teach_tasks", "train_tasks"))
    ...
    if not eval_tasks and nested_task_dict:
        eval_tasks = _extract_task_dict(
            nested_task_dict,
            preferred_keys=(
                "eval_same_map_no_tutor",
...
def _task_collection_to_dict(value: Any) -> dict[str, Any]:
    ...
    if isinstance(value, dict):
        # Nested split wrapper such as {"teach": [...], "eval_same_map_no_tutor": [...]}.
        if any(k in value for k in ("teach", "train", "eval", "eval_same_map", "eval_same_map_no_tutor")):
            out: dict[str, Any] = {}
            for key, nested in value.items():
                if key in {"teach", "train", "eval", "eval_same_map", "eval_same_map_no_tutor"}:
                    out.update(_task_collection_to_dict(nested))
            if out:
                return out
...
def _parse_coord(value: Any) -> Coord:
    return parse_runtime_coord(value)
