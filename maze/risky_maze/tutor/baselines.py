...
class AlwaysWaypointTutor:
    ...
    def act(self, *args: Any, **kwargs: Any) -> Any:
        ...
        wp = route[idx]
        base_dist = abs(start[0] - goal[0]) + abs(start[1] - goal[1])
        after_dist = abs(wp[0] - goal[0]) + abs(wp[1] - goal[1])
        progress = max(0.0, float(base_dist - after_dist) / max(1.0, float(base_dist)))
        return make_tutor_action(
            "WAYPOINT",
            waypoint=wp,
            reason="oracle_route_waypoint",
            diagnostics={
                "assist_leakage": 1.0,
                "base_assist_leakage": 1.0,
                "waypoint_novelty_leak": 1.0,
                "waypoint_progress_gift": progress,
            },
        )
