"""Final stabilization: intervention-rich diagnostic, p_self timing audit,
A1 replication, confidence binning.

Exp 1: Intervention-rich diagnostic (scripted tutor dose levels)
Exp 2: p_self timing audit (dc−dr conditioned)
Exp 3: A1 replication (T sweep)
Exp 4: Confidence binning calibration
"""
import sys
...
def run_session(lessons, theta, seed, n_teach=12, observer=None,
                force_dose=None, zero_pself=False):
    """Run session. force_dose overrides tutor dose for intervention-rich tests."""
    ...
    for step in range(n_teach):
        ...
        # Force dose for intervention-rich experiments
        if force_dose is not None:
            dose_oracle = force_dose
...
def main():
    ...
    L = ["# Shadow Observer — Final Stabilization\n\n"]

    # ─── Exp 1: Intervention-Rich Diagnostic ─────────────
    L.append("## Exp 1: Intervention-Rich Diagnostic\n\n")
    L.append("| Regime | Dose | Blind>0 | Mean blind | Corr_ν (A1) | Corr_ν (A2) |"
             " p_self real vs zero Δ |\n")
    L.append("|--------|:----:|:-------:|:----------:|:-----------:|:-----------:|"
             ":---------------------:|\n")
    print("Exp 1: Intervention-rich...", file=sys.stderr)
    for regime, fd in [("low (natural)", None), ("med (0.5)", 0.5), ("high (1.0)", 1.0)]:
        recs_a1 = []; recs_a2 = []; recs_a1z = []
        for th in ["safe", "shiny"]:
            for sid in range(NS):
                recs_a1.extend(run_session(ALL_LESSONS, th, sid, observer=A1MtObserver(), force_dose=fd))
                recs_a2.extend(run_session(ALL_LESSONS, th, sid, observer=A2MtObserver(), force_dose=fd))
                recs_a1z.extend(run_session(ALL_LESSONS, th, sid, observer=A1MtObserver(),
                                            force_dose=fd, zero_pself=True))
        m1 = analyze(recs_a1); m2 = analyze(recs_a2); m1z = analyze(recs_a1z)
        bv = [r["events"]["blind"] for r in recs_a1 if r.get("events")]
        n_pos = sum(1 for v in bv if v > 0)
        delta_nu = abs(m1["nu"]["corr"] - m1z["nu"]["corr"])
        L.append("| {} | {} | {} / {} | {:.4f} | {} | {} | {:.4f} |\n".format(
            regime, fd if fd else "nat", n_pos, len(bv), np.mean(bv),
            m1["nu"]["corr"], m2["nu"]["corr"], delta_nu))
        print(f"  {regime}: blind>0={n_pos}/{len(bv)} Corr_nu_A1={m1['nu']['corr']} "
              f"Corr_nu_A2={m2['nu']['corr']} Δ_pself={delta_nu:.4f}", file=sys.stderr)

    # ─── Exp 2: p_self timing audit ──────────────────────
    L.append("\n## Exp 2: p_self Timing-Conditioned Audit\n\n")
    L.append("| dc−dr | n_steps | mean_pself | blind>0 | selfdisc>0 | Corr_ν |\n")
    L.append("|:-----:|:-------:|:----------:|:-------:|:----------:|:------:|\n")
    print("\nExp 2: Timing audit...", file=sys.stderr)
    all_recs = []
    for th in ["safe", "shiny"]:
        for sid in range(NS):
            all_recs.extend(run_session(ALL_LESSONS, th, sid, n_teach=16,
                                        observer=A1MtObserver()))
    for gap_label, lo, hi in [("0", 0, 0), ("1-2", 1, 2), ("3+", 3, 99)]:
        sub = [r for r in all_recs if lo <= r.get("dc_minus_dr", 1) <= hi]
        if not sub: sub = all_recs  # fallback
        m = analyze(sub)
        ...
        n_bpos = sum(1 for v in bv if v > 0)
        n_spos = sum(1 for v in sv if v > 0)
        L.append("| {} | {} | {:.3f} | {} / {} | {} / {} | {} |\n".format(
            gap_label, len(sub), mean_ps, n_bpos, len(bv), n_spos, len(sv),
            m["nu"]["corr"]))

    # ─── Exp 3: A1 Replication ───────────────────────────
    L.append("\n## Exp 3: A1 Stability Replication\n\n")
    L.append("| T | Corr_τ | Corr_ν | Corr_γ | MAE_τ | ADR |\n")
    L.append("|:-:|:------:|:------:|:------:|:-----:|:---:|\n")
    print("\nExp 3: A1 replication...", file=sys.stderr)
    for n_t in [6, 12, 16, 20]:
        ...
        L.append("| {} | {} | {} | {} | {} | {} |\n".format(
            ...
            m["tau"]["mae"], m["ADR"]))
        print(f"  T={n_t}: Corr={m['tau']['corr']}/{m['nu']['corr']}/{m['gamma_gen']['corr']}", file=sys.stderr)

    # ─── Exp 4: Confidence Binning ───────────────────────
    L.append("\n## Exp 4: Confidence Binning Calibration (A1)\n\n")
    L.append("| Dim | Conf Bin | Mean Conf | Mean |error| | n |\n")
    L.append("|-----|:--------:|:---------:|:-------------:|:-:|\n")
    print("\nExp 4: Confidence binning...", file=sys.stderr)
    recs_cb = []
    for th in ["safe", "shiny"]:
        for sid in range(NS):
    ...
    for dim in ["tau", "nu", "gamma_gen"]:
        ...
        for lo, hi, label in [(0.0, 0.3, "low"), (0.3, 0.6, "mid"), (0.6, 1.0, "high")]:
            mask = (confs >= lo) & (confs < hi)
            if mask.sum() < 3: continue
            L.append("| {} | {} | {:.3f} | {:.4f} | {} |\n".format(
                dim, label, confs[mask].mean(), errs[mask].mean(), int(mask.sum())))

    rpt = out / "final_stabilization_results.md"
