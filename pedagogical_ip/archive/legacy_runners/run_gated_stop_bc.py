"""Gated STOP: Exp B (post-STOP info) + Exp C (cross-family) + Summary."""
import sys
...
def run_one(cct, th, seed, mt=12, ood_mode="none", held_out_family=None):
    ...
    return {"C":rate("B"),"E":rate("E"),"n_teach":nt,"n_eval":ne,
            "otr":otr["total"],"audit":au,"stop_info":stop_info}
...
def run_multi(th, seed, ns=4, **kw):
    ...
    rs=[]
    for s in range(ns):
        rs.append(run_one(cct,th,seed*10+s,ood_mode=kw.get('ood_mode','none'),held_out_family=kw.get('held_out_family')))
    return rs[-1]

def main():
    print("═══ Gated STOP: Exp B+C+Summary ═══\n",file=sys.stderr)
    L=["# Gated STOP: Exp B + C + Summary\n\n"]

    # Exp B: Post-STOP info (from controller trace)
    print("Exp B: Post-STOP diagnostic...",file=sys.stderr)
    L.append("## Exp B: Post-STOP Diagnostic\n\n")
    L.append("| θ | Mode | #T@STOP | margin | m_base | g_warm | g_plateau | Δu |\n|---|------|---------|--------|--------|--------|-----------|----|\n")
    for th in ["safe","shiny"]:
        for lab,warm,plateau,shared in [("single",False,False,True),("gated",True,True,False)]:
            stopinfos = []
            for sid in range(NS):
                r = run_multi(th,sid,warm=warm,plateau=plateau,shared_stop=shared)
                if r.get("stop_info"): stopinfos.append(r["stop_info"])
            if stopinfos:
                avg_nt = round(np.mean([s.get("n_teach",0) for s in stopinfos]),1)
                avg_margin = round(np.mean([s.get("margin",0) for s in stopinfos]),4)
                mb = sum(1 for s in stopinfos if s.get("m_base",True))/len(stopinfos)
                gw = sum(1 for s in stopinfos if s.get("g_warm",True))/len(stopinfos)
                gp = sum(1 for s in stopinfos if s.get("g_plateau",True))/len(stopinfos)
                avg_du = np.mean([s.get("delta_u",0) or 0 for s in stopinfos])
                L.append("| {} | {} | {} | {} | {:.0%} | {:.0%} | {:.0%} | {} |\n".format(
                    th,lab,avg_nt,avg_margin,mb,gw,gp,round(avg_du,6) if avg_du else "—"))
                print(f"  {th}×{lab}: #T@STOP={avg_nt} margin={avg_margin} g_warm={gw:.0%} g_plateau={gp:.0%}",file=sys.stderr)
            else:
                L.append("| {} | {} | — | — | — | — | — | — |\n".format(th,lab))
                print(f"  {th}×{lab}: no STOP",file=sys.stderr)

    # Exp C: Cross-family robustness
    print("\nExp C: Cross-family...",file=sys.stderr)
    ...
    # OOD credibility
    print("\nOOD credibility...",file=sys.stderr)
    L.append("\n## OOD Credibility (Gated STOP)\n\n")
    L.append("| θ | OOD | **C** | **E** | OTR |\n|---|-----|---|---|---|\n")
    for th in ["safe","shiny"]:
        for ood in ["none","sign_flip","noise_heavy"]:
            rs=[run_multi(th,sid,warm=True,plateau=True,ood_mode=ood) for sid in range(NS)]
    ...
    for th in ["safe","shiny"]:
        ...
        print(f"  {th}: v13 C={sf(c_old)} → gated C={sf(c_new)} #T {sf(nt_old,'{:.0f}')}→{sf(nt_new,'{:.0f}')}",file=sys.stderr)
