"""CCT-v9: Contextual Constrained Bayesian — Full Experiment.

Exp A: v9 vs v8 vs v7 vs v6 vs fixed (5 strats × 2θ × 8 seeds, 4 sessions)
Exp B: Term actionability audit (AM, PCR per term)
Exp C: Constraint ablation (constraint vs penalty vs none)
...
def run_one_session(cct, strategy, theta, seed, max_teach=12, budget=4.0):
    ...
    traces = {"A":[],"B":[],"C":[],"D":[],"E":[]}; lf_scores = []; n_teach = 0; n_eval = 0
    stopped_at = None; idx = 0; fv = None
    for step in range(max_teach + 4):
        if cct: action_type, lesson, q_val, info = cct.select_action(m)
        else: action_type, lesson, q_val = "TEACH", fixed_lesson(strategy, rng), 0
        if action_type == "STOP": stopped_at = step; break
        if action_type == "EVAL":
            n_eval += 1; probes = all_probes(m, AP, theta); cct.update_mastery(probes); continue
        n_teach += 1
        if n_teach > max_teach: break
        u_before = cct.mastery.mastery() if cct else {p: 0.5 for p in ["RC","TR","EP","VA","IA"]}
        nu_b = m.nu; gg_b = m.gamma_gen; otr_b = overteach_rate_v2(m)["total"]
        ...
        lf_scores.append(ep_params.fidelity_to(type('L', (), {'subtype': lesson.subtype, 'severity': lesson.severity, 'dose_profile': lesson.dose_profile, 'family': lesson.family})()))
        ...
        if ep_params.subtype == "beneficial_novelty": nf = [False, True] if sc.oracle_safe_branch_id == 1 else [True, False]
        ...
        elif not chose_risky: m.update_dependence(self_discovery=True); m.update_gamma_gen(successful_exploration=True)
        ...
        bridge.update(m, probes, sc.risk_level if hasattr(sc, 'risk_level') else 0.3, bar.temptation_score, ep_params.novelty, 0.7 if has_self_ev else 0.3)
        if cct and hasattr(cct, 'update_response'):
            otr_a = overteach_rate_v2(m)["total"]
            try: cct.update_response(lesson.name, dict(u_before), cct.mastery.mastery(), nu_b, m.nu, gg_b, m.gamma_gen, otr_b, otr_a)
            except TypeError: cct.update_response(lesson.name, dict(u_before), cct.mastery.mastery())
    ...
    otr = overteach_rate_v2(m); avg_lf = round(np.mean(lf_scores), 3) if lf_scores else None
    ...
    return {"B": rate("B"), "C": rate("C"), "D": rate("D"), "E": rate("E"),
            "mca_E": rate("E", "mca"), "LF": avg_lf, "n_teach": n_teach, "n_eval": n_eval,
            ...
            "otr": otr["total"], "se": round(se, 4), "post_up": post_up, "audit": audit}
...
def make_cct(strategy, theta, budget, ablation=None):
    if strategy == "cct_v9" or (ablation and ablation not in ("v8",)):
        cct = CurriculumControllerV9(theta=theta, total_budget=budget)
...
def main():
    print("═══ CCT-v9: Contextual Constrained Bayesian ═══\n", file=sys.stderr)
    lines = ["# CCT-v9: Contextual Constrained Bayesian Planner\n\n"]
    # ─── Exp A ───
    strats = ["ppmrb_only", "tic_heavy", "cct_v8", "cct_v9"]
    lines.append("## Exp A: CCT-v9 vs v8 vs Fixed (4 sessions)\n\n")
    ...
    lines.append("| θ | Term | AM (variance) | PCR (policy change) |\n")
    lines.append("|---|------|--------------|--------------------|\n")
    for theta in ["safe", "shiny"]:
        rs = [run_multi("cct_v9", theta, sid) for sid in range(8)]
        all_am = {}; all_pcr = {}
        for r in rs:
            a = r.get("audit", {})
            for tn, v in a.get("AM", {}).items(): all_am.setdefault(tn, []).append(v)
            for tn, v in a.get("PCR", {}).items(): all_pcr.setdefault(tn, []).append(v)
        for tn in ["G_C","G_E","U","harm_otr","harm_nu","harm_gg"]:
    ...
    # ─── Exp C: Constraint ───
    print("\nExp C: Constraint...", file=sys.stderr)
    lines.append("\n## Exp C: Constraint Ablation\n\n")
    lines.append("| θ | Condition | **C** | **E** | SE | OTR | ν |\n")
    lines.append("|---|----------|---|---|---|---|---|\n")
    for theta in ["safe", "shiny"]:
        for abl in [None, "no_constraint", "no_unc", "no_stop"]:
            label = abl if abl else "full (constraint)"
            rs = [run_multi("cct_v9", theta, sid, ablation=abl) for sid in range(8)]
    ...
    for theta in ["safe", "shiny"]:
        for bud in [2.0, 4.0, 8.0]:
            rs = [run_multi("cct_v9", theta, sid, budget=bud) for sid in range(8)]
    ...
    with open(out / "cct_v9_report.md", "w") as f: f.writelines(lines)
    print(f"\nReport -> results/cct_v9_report.md", file=sys.stderr); print("Done.", file=sys.stderr)
